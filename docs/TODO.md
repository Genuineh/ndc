...
| **P1-TuiCrate** | 📋 规划完成 | TUI 独立 Crate 提取（ndc-tui） |
| **P1** | 待开始 | 核心自治能力与治理 |
...
| Phase | 状态 | 内容 |
|-------|------|------|
| Phase 1 | 待开始 | 前置解耦：redaction 迁移至 ndc-core + 定义 AgentBackend trait |
| Phase 2 | 待开始 | Crate 创建与文件迁移（ndc-tui 骨架 + 代码搬迁 + 引用更新） |
| Phase 3 | 待开始 | Interface 适配（impl AgentBackend for AgentModeManager + repl.rs 更新） |
| Phase 4 | 待开始 | 验证与清理（全量测试 + clippy + 文档同步） |
