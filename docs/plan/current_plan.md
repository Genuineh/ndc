...
2. P0-A/B/C/D 已全部完成，P1-UX-1~6 已全部完成。
3. P0-SEC 深度安全审计已全部完成（Immediate/Short/Medium/Structural 四层 20+ 修复项，+80 新测试）。
4. 最近 BugFix：
   - Shell 执行命令修复（`sh -c` 委托执行完整命令字符串）
   - Ctrl+C 中断运行任务（不再直接退出 REPL）
5. **已完成**: P1-Scene（Context-Aware Adaptive Session TUI — 方案 A 精简）
   - Phase 1 ✅: repl.rs 提取为 `tui/` 模块（9 子模块，5301→268 行，153 测试分布于各模块）
   - Phase 2 ✅: Scene 渲染提示（`tui/scene.rs` ~224 行，12 单元测试）
   - Phase 3 ✅: DiffPreview + 工具类型强调色（10 新测试）
